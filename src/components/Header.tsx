        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <div className="relative group" id="role-selector-dropdown">
            <button
              ...
              className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-gradient-to-r from-blue-600/20 to-indigo-600/20 border border-blue-500/30 text-white text-xs font-semibold cursor-pointer hover:border-blue-400 transition-colors"
            >
              <UserCheck className="w-3.5 h-3.5 text-cyan-400" />
              <span>{roles.find((r) => r.value === currentRole)?.label || 'Switch Portal'}</span>
              <ChevronDown className="w-3.5 h-3.5 text-slate-400" />
            </button>

            {/* Dropdown Menu */}
            <div
              className={`absolute right-0 mt-1 w-48 bg-slate-900 border border-slate-700 rounded-xl shadow-2xl p-1.5 z-50 transition-all ${
